...
// dd($contact);
        foreach ($data as $key => $value) {
            ...
            if (strpos($value, '{contact_name}') !== false) {
                //#king corrected contact name not showing when its a business name
                $contact_name = !empty($transaction->contact->name) ? 
                $transaction->contact->name : $transaction->contact->supplier_business_name;
            ...
            if (! empty($transaction)) {
                foreach ($transaction->payment_lines as $payment) {
                    if ($payment->is_return != 1) {
                        ...
                        //#king added received amount to be receieved amount instead of total amount
                        $paid = $payment->amount;
                    }
                }
            }
            ...
            $paid_now = $this->num_f($paid, true, $business->currency);
// dd($paid_now);
            //Replace paid_amount
            ...
            if (strpos($value, '{received_amount}') !== false) {
                $data[$key] = str_replace('{received_amount}', $paid_now, $data[$key]);
            }
